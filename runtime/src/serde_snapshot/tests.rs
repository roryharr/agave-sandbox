...
mod serde_snapshot_tests {
    ...
    fn check_ref_count(
        ...
        expected_ref_count: u64,
        actual_ref_count: u64,
    ) {
    ...
    fn test_accounts_db_serialize1(storage_access: StorageAccess) {
        for pass in 0..2 {
            ...
            // With accounts marked obsolete in the storages, storages are shrunk when saved/restored
    ...
    fn test_accounts_clean_after_snapshot_restore_then_old_revives(storage_access: StorageAccess) {
        ...
        // Shrink is needed if obsolete accounts is enabled
        // A shrink at any point after step D will remove the account from the index
