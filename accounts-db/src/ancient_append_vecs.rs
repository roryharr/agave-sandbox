...
use {
    crate::{
        ...
        accounts_db::{
            AccountFromStorage, AccountsDb, AliveAccounts, GetUniqueAccountsResult,
            SHRINK_COLLECT_CHUNK_SIZE, ShrinkCollect,
            stats::{ShrinkAncientStats, SquashStatsSub},
        },
        accounts_index::ScanFilter,
        active_stats::ActiveStatItem,
        storable_accounts::{StorableAccounts, StorableAccountsBySlot},
        u64_align,
    },
    rand::{Rng, rng},
    rayon::prelude::{IntoParallelRefIterator, ParallelIterator, ParallelSlice},
    solana_clock::Slot,
...
impl AccountsDb {
    ...
    fn combine_ancient_slots_packed_internal(
        ...
        // we want to put the accounts from the LARGEST storages at the end.
    ...
    /// collect the alive accounts of each storage and assert that every one can be moved
    /// to any slot in the combine range
    fn calc_accounts_to_combine<'a>(
        ...
        let max_ancient_slot = accounts_per_storage
            .first()
            .map(|(info, _unique_accounts)| info.slot)
            .unwrap_or_default();
    ...
    /// Assert that every account in `alive_accounts` can be moved to any slot up to
    /// `max_ancient_slot`.
    /// An alive account in an ancient storage may have other index entries only at slots newer
    /// than the ancient range: a newer version whose reclaim of this copy clean has not yet
    /// performed. An entry at another ancient slot would mean clean failed to reclaim a
    /// duplicate, and packing could reorder the account's entries across it.
    fn assert_ancient_accounts_can_be_moved(
        &self,
        alive_accounts: &AliveAccounts,
        max_ancient_slot: Slot,
    ) {
        let slot = alive_accounts.slot;
        self.thread_pool_background.install(|| {
            alive_accounts
                .accounts
                ...
                .for_each(|accounts| {
                    self.accounts_index.scan(
                        ...
                        |pubkey, slots_refs| {
                            let (slot_list, _ref_count) = slots_refs.unwrap_or_else(|| {
                                panic!("no index entry for alive account {pubkey} at slot {slot}")
                            });
                            assert!(
                                slot_list
                                    .iter()
                                    .any(|(entry_slot, _info)| *entry_slot == slot),
                                "alive account {pubkey} has no index entry at slot {slot}"
                            );
                            for (entry_slot, _info) in slot_list {
                                assert!(
                                    *entry_slot == slot || *entry_slot > max_ancient_slot,
                                    "account {pubkey} at slot {slot} has another index entry at \
                                     ancient slot {entry_slot}"
                                );
                            }
                        },
                        ScanFilter::All,
...
mod tests {
                ...
                for unsorted_slots in [false, true] {
                    ...
                    if add_dead_account {
                        storages.iter().for_each(|storage| {
                            ...
                            let account_offset = db.accounts_index.get_and_then(&pk, |entry| {
                    ...
                    // all slots are available as target slots
                    let expected_target_slots_sorted = if unsorted_slots {
                    ...
                    // every storage's single alive account is packable
                    assert!(
                        accounts_to_combine
                            .accounts_to_combine
                            .iter()
                            .all(
                                |shrink_collect| shrink_collect.alive_accounts.accounts.len() == 1
                            )
                    );

                    let write_ancient_accounts =
                        db.write_packed_storages(&accounts_to_combine, Vec::default());
                    assert!(write_ancient_accounts.shrinks_in_progress.is_empty());
                }
            }
        }
    ...
    fn test_calc_accounts_to_combine_older_dup(accounts_db_config: AccountsDbConfig) {
        ...
        // with 2 accounts
        // 1 with 1 ref
        // 1 with 2 refs, where the other entry is at a newer slot outside the ancient range
        // The account with 2 refs is packable: its newer entry stays the newest one in the
        // index no matter which ancient slot the old copy moves to.
        let db = AccountsDb::new_for_tests_with_config(Vec::new(), accounts_db_config);
        let num_slots = 1;
        ...
        let accounts_to_combine = db.calc_accounts_to_combine(&mut accounts_per_storage);
        let slots_vec = slots.collect::<Vec<_>>();
        ...
        // both accounts, including the one with a newer duplicate, are packable
        let mut alive_pubkeys = accounts_to_combine
            .accounts_to_combine
            .first()
            .unwrap()
            .alive_accounts
            ...
            .collect::<Vec<_>>();
        alive_pubkeys.sort_unstable();
        let mut expected_pubkeys = vec![*pk_with_2_refs, pk_with_1_ref];
        expected_pubkeys.sort_unstable();
        assert_eq!(alive_pubkeys, expected_pubkeys);

        // pack and write everything; both accounts land in the packed storage
        let pack = PackedAncientStorage::pack(
            accounts_to_combine
                .accounts_to_combine
                .iter()
        ...
        let write_ancient_accounts = db.write_packed_storages(&accounts_to_combine, pack);
        ...
        let mut written_pubkeys = Vec::new();
        shrink_in_progress
            .new_storage()
            .accounts
            .scan_accounts(&mut reader, |_offset, account| {
                written_pubkeys.push(*account.pubkey());
            })
            .expect("must scan accounts storage");
        written_pubkeys.sort_unstable();
        assert_eq!(written_pubkeys, expected_pubkeys);
    }

    #[test_case(ACCOUNTS_DB_CONFIG_APPEND_VEC)]
    #[should_panic(expected = "another index entry at ancient slot")]
    fn test_calc_accounts_to_combine_ancient_dup_panics(accounts_db_config: AccountsDbConfig) {
        // an alive account with a second index entry at another ancient slot means clean
        // failed to reclaim one of the copies; packing must not move it
        let db = AccountsDb::new_for_tests_with_config(Vec::new(), accounts_db_config);
        let (storages, _slots, infos) = get_sample_storages(&db, 1, None);
