...
struct IndexGenerationAccumulator {
    ...
    /// Pubkeys of the zero-lamport accounts encountered while scanning. After older versions are
    /// resolved, those whose surviving entry is still a single-ref zero-lamport are removed from
    /// the index and recorded as tombstones.
...
impl AccountsDb {
    ...
    fn remove_uncleaned_slots_up_to_slot_and_move_pubkeys(
        ...
        candidates: &[RwLock<HashSet<Pubkey>>],
    ) {
    ...
    fn count_pubkeys(candidates: &[RwLock<HashSet<Pubkey>>]) -> u64 {
        candidates
            .iter()
                    ...
                    self.accounts_index
                        .get_and_then(entry.key(), |index_entry| {
                            if let Some(index_entry) = index_entry {
                                match (index_entry.slot_list_lock_read_len() as usize).cmp(&entry.value().len()) {
                                    ...
                                    std::cmp::Ordering::Greater => {
                                        ...
                                        if ((index_entry.slot_list_lock_read_len() as usize) - num_too_new)
                                            > entry.value().len()
                                        {
                                            failed.store(true, Ordering::Relaxed);
                                            error!(
                                                "exhaustively_verify_refcounts: too \
    ...
    fn load_accounts_index_for_shrink<'a, T: ShrinkCollectRefs<'a>>(
        ...
        self.accounts_index.scan(
            ...
            |_, slots_refs| {
                ...
                let mut do_populate_accounts_for_shrink = |slot_list: &[(u64, AccountInfo)]| {
                    if stored_account.is_zero_lamport() && slot_list.len() == 1 {
                        panic!();
                    } else {
                ...
                if let Some(slot_list) = slots_refs {
                    ...
                    assert!(is_alive);
                    do_populate_accounts_for_shrink(&slot_list);
                } else {
    ...
    pub(crate) fn shrink_collect<'a: 'b, 'b, T: ShrinkCollectRefs<'b>>(
        ...
        let can_purge_tombstones = self.can_purge_tombstones_after_shrink(slot);
    ...
    fn do_flush_slot_cache(
        ...
        if !accounts.is_empty() {
            ...
            if self.storage.get_slot_storage_entry(slot).is_some() {
                self.reopen_storage_as_readonly_shrinking_in_progress_ok(slot);
            }
        }
    ...
    /// Can tombstones in `slot` be purged?
    fn can_purge_tombstones_after_shrink(&self, slot: Slot) -> bool {
    ...
    pub(crate) fn alive_bytes_after_shrink(&self, store: &AccountStorageEntry) -> usize {
        ...
        if self.can_purge_tombstones_after_shrink(store.slot()) {
            store.alive_bytes_exclude_tombstones()
        } else {
            store.alive_bytes()
        }
    }
        ...
        I: Iterator<Item = &'a (Slot, AccountInfo)>,
    {
        ...
        reclaimed_offsets.into_iter().for_each(|(slot, offsets)| {
            // Special case: if reclaims are being done on the slot itself, then these accounts
            // are tombstones, not obsolete accounts. Mark them as such.
            if let MarkAccountsObsolete::Yes(slot_marked_obsolete) = mark_accounts_obsolete
                && slot == slot_marked_obsolete
            {
                let store = self
                    .storage
                    .get_slot_storage_entry(slot)
                    .expect("storage must exist for a slot with an indexed account");
                store.batch_insert_tombstone_offsets(offsets);
                if store.has_only_tombstones() {
                    // Every remaining alive account is a tombstone with no index entry
    ...
    fn generate_index_for_slot<'a>(
        ...
        let zero_lamport_pubkeys = &mut accum.zero_lamport_pubkeys;
        ...
        let num_obsolete_accounts_skipped = storage
            .scan_accounts(reader, |offset, account| {
                ...
                if !is_account_zero_lamport {
                    ...
                    // storage) once their older versions are resolved. Collect their pubkeys so the
                    // tombstone pass after obsolete-marking can find and remove the survivors.
    ...
    pub fn generate_index(
        ...
        timings.num_tombstones_created = self
            .create_tombstones_at_startup(std::mem::take(&mut total_accum.zero_lamport_pubkeys));
    ...
    /// remove those whose surviving entry is still a single-ref zero-lamport account from the index
    /// and record them as tombstones on their storages. Must run after older versions have been
    /// obsolete-marked so that each surviving zero-lamport is genuinely single-ref. Returns the
    /// number of tombstones created.
    fn create_tombstones_at_startup(&self, zero_lamport_pubkeys: Vec<Pubkey>) -> u64 {
        if zero_lamport_pubkeys.is_empty() {
            return 0;
        }

        // Group by index bin so each bin's entries are removed while holding that bin's lock once.
        let mut pubkeys_by_bin: Vec<Vec<Pubkey>> = vec![Vec::new(); self.accounts_index.bins()];
        for pubkey in zero_lamport_pubkeys {
            let bin = self.accounts_index.bin_calculator.bin_from_pubkey(&pubkey);
            pubkeys_by_bin[bin].push(pubkey);
        }
