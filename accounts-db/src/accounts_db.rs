...
use {
    crate::{
        ...
        accounts_index::{
            ...
            ReclaimsSlotList, ReclaimsWithNewestSlot, ScanFilter, SlotList, Startup,
...
impl AccountsDb {
    ...
    fn update_candidate_after_reclaims(
        ...
        // Mark any ZLSRs
        if candidate_info.slot_list.len() == 1
                ...
                while !pending_stores.is_empty() {
                    ...
                    if Some(slot) == min_slot {
                        if let Some(failed_slot) = failed_slot.take() {
                            ...
                            info!(
                                "calc_delete_dependencies, oldest slot is not able to be deleted \
                                 because of {pubkey}, slot list len: {}", slot_list.len()
                            );
                        }
                    }
    ...
    pub fn clean_accounts(&self, max_clean_root_inclusive: Option<Slot>, is_startup: bool) {
        ...
        if !reclaims.is_empty() {
            // Collect the expected dead slots
            let expected_dead_slots = reclaims.iter().map(|(slot, _)| *slot).collect();
            ...
            assert_eq!(dead_slots, expected_dead_slots);
        }
    ...
    {
        let dead_slots =
            self.remove_dead_accounts(reclaims, mark_accounts_obsolete);
    ...
    fn load_accounts_index_for_shrink<'a, T: ShrinkCollectRefs<'a>>(
        ...
        self.accounts_index.scan(
            ...
            |pubkey, slots_refs| {
                ...
                let mut do_populate_accounts_for_shrink = |slot_list: &[(Slot, AccountInfo)]| {
                ...
                if let Some(slot_list) = slots_refs {
                    ...
                    assert!(is_alive, "The Index must contain an entry for this slot that has not been marked obsolete");
    ...
    /// These accounts were found during shrink of `slot` to be slot_list=[slot] and lamports = 0.
    ...
    /// Remove `slot` from the index entry's slot list. then the pubkey can be removed completely from the index.
    ...
    fn purge_slot_storage(&self, remove_slot: Slot, purge_stats: &PurgeStats) {
        ...
        if !reclaims.is_empty() {
            ...
            let expected_dead_slot = IntSet::from_iter([remove_slot]);
            assert_eq!(dead_slots, expected_dead_slot);
        }
