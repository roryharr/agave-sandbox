...
impl AccountsDb {
    ...
    pub fn purge_keys_exact<C>(&self, pubkey_to_slot_set: impl IntoIterator<Item = (Pubkey, C)>)
    where
        C: for<'a> Contains<'a, Slot>,
    {
        ...
        let (_, purge_exact_us) =
            measure_us!(for (pubkey, slots_set) in pubkey_to_slot_set.into_iter() {
                purge_exact_count += 1;
                let is_empty = self.accounts_index.purge_exact(&pubkey, slots_set);
                if is_empty {
                    dead_keys.push(pubkey);
                }
            });

        let (_, handle_dead_keys_us) = measure_us!(self
            .accounts_index
    ...
    pub fn clean_accounts(&self, max_clean_root_inclusive: Option<Slot>, is_startup: bool) {
        ...
        let mut slot_set = IntSet::default();
    ...
    fn purge_slot_cache_pubkeys(
        ...
        assert!(self
            ...
            .is_none());
        self.purge_keys_exact(pubkeys.into_iter().map(|key| (key, purged_slot)));
        if is_dead {
    ...
    fn purge_slot_storage(&self, remove_slot: Slot, purge_stats: &PurgeStats) {
        ...
        self.purge_keys_exact(stored_keys);
        ...
        let mut slot_to_remove = IntSet::default();
        slot_to_remove.insert(remove_slot);
        self.process_dead_slots(&slot_to_remove, None, purge_stats, false);
    ...
    fn unref_pubkeys<'a>(
        ...
        self.thread_pool_background.install(|| {
            (0..batches).into_par_iter().for_each(|batch| {
                ...
                self.accounts_index.scan(
                    pubkeys.clone().skip(skip).take(UNREF_ACCOUNTS_BATCH_SIZE),
                    |_pubkey, slots_refs| {
                        if let Some((slot_list, ref_count)) = slots_refs {
                            // Let's handle the special case - after unref, the result is a single ref zero lamport account.
                            if slot_list.len() == 1 && ref_count == 2 {
    ...
    fn clean_stored_dead_slots(
        ...
        if let Some(purged_account_slots) = purged_account_slots {
            self.unref_accounts(purged_slot_pubkeys, purged_account_slots);
        }
        measure_unref.stop();
