...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> AccountsIndex<T, U> {
    ...
    /// Dispatch an indexed scan to the appropriate secondary index.
    /// Does not manage the scan lifecycle — the caller is responsible for
    /// creating/finishing a `ScanGuard`.
    pub(crate) fn do_scan_indexed_accounts<F>(
        &self,
        ancestors: &Ancestors,
        func: F,
        index_key: &IndexKey,
    ...
    pub(crate) fn do_scan_accounts<F, R>(
        &self,
    ...
    pub(crate) fn do_scan_secondary_index<
        F,
    ...
    pub(crate) fn scan_accounts<F>(
        ...
        F: FnMut(&Pubkey, (&T, Slot)),
    {
        let max_root = self.max_root_inclusive();
