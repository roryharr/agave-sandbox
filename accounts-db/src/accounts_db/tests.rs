...
fn test_shrink_unref_handle_zero_lamport_single_ref_accounts() {
    ...
    assert!(db.dirty_stores.read().unwrap().contains_key(&1));
    assert!(!db.shrink_candidate_slots.lock().unwrap().contains(&1));
...
define_accounts_db_test!(test_mark_dirty_dead_stores_empty, |db| {
    ...
    for add_dirty_stores in [false, true] {
        ...
        assert!(db.dirty_stores.read().unwrap().is_empty());
    }
});
...
fn test_mark_dirty_dead_stores_no_shrink_in_progress() {
    ...
    for add_dirty_stores in [false, true] {
        ...
        if add_dirty_stores {
            assert_eq!(1, db.dirty_stores.read().unwrap().len());
            let dirty_store_id= db.dirty_stores.read().unwrap().get(&slot).unwrap().id();
...
fn test_mark_dirty_dead_stores() {
    ...
    for add_dirty_stores in [false, true] {
        ...
        if add_dirty_stores {
            assert_eq!(1, db.dirty_stores.read().unwrap().len());
            let dirty_store_id = db.dirty_stores.try_read().unwrap().get(&slot).unwrap().id();
            assert_eq!(dirty_store_id, old_id);
        } else {
            assert!(db.dirty_stores.read().unwrap().is_empty());
        }
...
fn test_clean_old_storages_with_reclaims_rooted() {
    ...
    for &slot in &slots {
        ...
        assert!(!accounts_db.dirty_stores.read().unwrap().contains_key(&slot));
    ...
    let old_storage = accounts_db
        ...
        .unwrap();
    accounts_db.dirty_stores.write().unwrap().insert(old_slot, old_storage);
...
fn test_clean_old_storages_with_reclaims_unrooted() {
    ...
    assert!(!accounts_db.dirty_stores.read().unwrap().contains_key(&old_slot));
    // and `new_slot` should be in neither
    assert!(!accounts_db.uncleaned_pubkeys.contains_key(&new_slot));
    assert!(!accounts_db.dirty_stores.read().unwrap().contains_key(&new_slot));
