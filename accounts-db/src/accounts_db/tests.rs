...
fn test_load_after_remove_unrooted_and_restore_to_same_slot() {
    ...
    // Load thread – continuously attempts to load the account.  Randomise load_limit
    // to exercise both the fast and slow retry paths.
    let t_do_load = {
        let db = db.clone();
        let exit = exit.clone();
        std::thread::Builder::new()
    ...
    // Prior to the fix, it failed with a panic in 'retry-toget-account-accessor' after ~1 second,
