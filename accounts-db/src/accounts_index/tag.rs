//! The accounts index is keyed by a `Tag`, a 64 bit hash of the pubkey, rather than by
//! the pubkey itself. That takes a map slot from 40 bytes (32 pubkey + 8 entry) down to 16.
//!
//! The seed is random per boot, so tags are not stable across restarts and cannot be ground
//! offline.
//!
//! Two pubkeys that share a tag share an index entry. That is not handled: at mainnet scale
//! (~1.2 billion accounts) the chance of any colliding pair is ~3.6% per boot. A collision
//! surfaces as a bogus duplicate during index generation or as the load path's pubkey assert,
//! and a restart reseeds. That is acceptable for this proof of concept; productizing this
//! handles collisions the way hash tables do: probe a next tag, verifying the pubkey against
//! the account record each entry points at.
...
/// The key of the accounts index: a 64 bit hash of a pubkey.
pub type Tag = u64;

/// Computes the `Tag` for a pubkey, using a seed chosen per boot.
#[derive(Debug, Clone)]
...
///
/// A tag is a hash, so the map's bucket index (the low bits), the shard choice (the high
/// bits), and hashbrown's control byte are all already well distributed. Hashing it again
...
impl Hasher for TagHasher {
    ...
    fn write(&mut self, _bytes: &[u8]) {
        unreachable!("the accounts index is keyed by Tag")
    }
