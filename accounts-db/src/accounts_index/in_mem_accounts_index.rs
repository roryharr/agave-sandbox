...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> InMemAccountsIndex<T, U> {
    ...
    /// and the entry is additionally flushed to disk immediately and the dirty
    /// flag may be cleared.
    ...
    /// If the in-mem entry for pubkey is `slot_list.len() == 1` and currently dirty, write
    /// it through to disk
    ...
    fn gather_possible_flush_evict_candidates<'a>(
        ...
        for (k, v) in iter {
            ...
            // Skip entries with slot_list_read_lock != 1 early
            if v.slot_list_lock_read_len() != 1 {
                continue;
            }
...
mod tests {
    ...
    /// `slot_list_mut` write-through fires only for `slot_list.len() == 1`
